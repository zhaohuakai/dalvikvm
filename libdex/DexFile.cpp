...
{
	//printf("ZHK in DexFile.cpp:291\n");
    DexFile* pDexFile = NULL;
    const DexHeader* pHeader;
    const u1* magic;
    int result = -1;
	//printf("ZHK in DexFile.cpp:295\n");
    if (length < sizeof(DexHeader)) {
		printf("ZHK in DexFile.cpp:297 too short to be a valid .dex\n");
    ...
    if (memcmp(data, DEX_OPT_MAGIC, 4) == 0) {
        ...
        if (memcmp(magic+4, DEX_OPT_MAGIC_VERS, 4) != 0) {
			//printf("ZHK in DexFile.cpp:313 bad opt version (0x%02x %02x %02x %02x)\n",
		...
		//printf("ZHK in DexFile.cpp:321 Good opt header, DEX offset is %d, flags=0x%02x\n",
	...
	printf("ZHK in DexFile.cpp:340\n");
	// 后面出错
    dexFileSetupBasicPointers(pDexFile, data);
    pHeader = pDexFile->pHeader;
	printf("ZHK in DexFile.cpp:345\n");
	// TODO
    if (!dexHasValidMagic((printf("****\n"),pHeader))) {
        goto bail;
    }
	printf("ZHK in DexFile.cpp:349\n");
    /*
