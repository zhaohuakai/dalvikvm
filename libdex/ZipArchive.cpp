        ...
        ZipArchive* pArchive, off_t fileLength, size_t readAmount, u1* scanBuf)
{
	...
	printf("ZHK in ZipArchive.cpp:150 searchStart=%d, fileLength=%d, readAmount=%d\n", searchStart, fileLength, readAmount);
    long posi = lseek(fd, searchStart, SEEK_SET);
	printf("ZHK in ZipArchive.cpp:152 posi=%d\n", posi);
	if (posi != searchStart) {
		printf("ZHK in ZipArchive.cpp:154 Zip: seek %ld failed: %s\n", (long) searchStart, strerror(errno));
	...
	printf("ZHK in ZipArchive.cpp:183\n");
    /*
    ...
    ZipArchive* pArchive)
{
	...
	int iresult;
    struct stat buf;
    iresult = stat(debugFileName,&buf);
    if(iresult == 0)
    {
    ...
    */  
	...
	// ZHKTODO
	//off_t fileLength = 753;
	printf("ZHK in ZipArchive.cpp:245 length %ld  zip\n", (long) fileLength);
    if (fileLength < kEOCDLen) {
		printf("ZHK in ZipArchive.cpp:254 Zip: length %ld is too small to be zip\n", (long) fileLength);
...
{
    int fd, err;
	printf("ZHK in ZipArchive.cpp:350 Opening as zip '%s'\n", fileName);
    ALOGV("Opening as zip '%s' %p", fileName, pArchive);

    memset(pArchive, 0, sizeof(ZipArchive));
	fd = open(fileName, O_RDONLY | O_BINARY, 0);
	printf("ZHK in ZipArchive.cpp:356 fd=%d\n",fd);
    if (fd < 0) {
        err = errno ? errno : -1;
		printf("ZHK in ZipArchive.cpp:360 Unable to open '%s': %s\n", fileName, strerror(err));
...
int dexZipPrepArchive(int fd, const char* debugFileName, ZipArchive* pArchive)
{
    ...
    if (mapCentralDirectory(fd, debugFileName, pArchive) != 0){
		printf("in ZipArchieve.cpp:380\n");
		goto bail;
	}
    if (parseZipArchive(pArchive) != 0) {
		printf("in ZipArchieve.cpp:380 Zip: parsing '%s' failed\n", debugFileName);
...
{
	printf("ZHK in ZipArchive.cpp:426\n");
	printf("ZHK in ZipArchive.cpp:427 Closing archive %p\n", pArchive);
    ALOGV("Closing archive %p", pArchive);

    if (pArchive->mFd >= 0)
        close(pArchive->mFd);
	printf("ZHK in ZipArchive.cpp:432\n");
    sysReleaseShmem(&pArchive->mDirectoryMap);
	printf("ZHK in ZipArchive.cpp:434\n");
    free(pArchive->mHashTable);
	printf("ZHK in ZipArchive.cpp:436\n");
	...
	printf("ZHK in ZipArchive.cpp:443\n");
}
    ...
    const ZipEntry entry, int fd)
{
    ...
    int ent = entryToIndex(pArchive, entry);
    if (ent < 0) {
		printf("Zip: extract can't find entry %p", entry);
    ...
    if (lseek(pArchive->mFd, dataOffset, SEEK_SET) != dataOffset) {
		printf("Zip: lseek to data at %ld failed", (long) dataOffset);
