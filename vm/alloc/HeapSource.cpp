                             ...
                             size_t growthLimit)
{
    ...
    if (!(startSize <= growthLimit && growthLimit <= maximumSize)) {
		...
		printf("ZHK in HeapSource.cpp:517 \n");
        return NULL;
    }
	...
	base = dvmAllocRegion(length, PROT_NONE, "/dev/zero");
	//printf("ZHK in HeapSource.cpp:528 \n");
    if (base == NULL) {
		printf("ZHK in HeapSource.cpp:530 \n");
        return NULL;
    }
	//printf("ZHK in HeapSource.cpp:533 \n");
	// 划分一块内存来当heap使用
    /* Create an unlocked dlmalloc mspace to use as
     * a heap source.
     */
    ...
    if (msp == NULL) {
		printf("ZHK in HeapSource.cpp:537 \n");
        goto fail;
    }
	//printf("ZHK in HeapSource.cpp:543 \n");
    gcHeap = (GcHeap *)calloc(1, sizeof(*gcHeap));
    if (gcHeap == NULL) {
        LOGE_HEAP("Can't allocate heap descriptor");
		printf("ZHK in HeapSource.cpp:544 \n");
        goto fail;
    }
	//printf("ZHK in HeapSource.cpp:550 \n");
    hs = (HeapSource *)calloc(1, sizeof(*hs));
    if (hs == NULL) {
        LOGE_HEAP("Can't allocate heap source");
        free(gcHeap);
		printf("ZHK in HeapSource.cpp:552 \n");
        goto fail;
    }
    ...
    if (!allocMarkStack(&gcHeap->markContext.stack, hs->maximumSize)) {
        ...
        dvmHeapBitmapDelete(&hs->liveBits);
		printf("ZHK in HeapSource.cpp:587 Can't create markStack");
        goto fail;
    }
	//printf("ZHK in HeapSource.cpp:593 \n");
...
fail:
    munmap(base, length);
	printf("ZHK in HeapSource.cpp:602 return NULL");
    return NULL;
}
