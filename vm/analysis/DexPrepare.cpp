...
{
	printf("ZHK in DexPrepare.cpp:132\n");
    int fd, cc;
...
retry:
	...
	printf("in Dexprepare.cpp:168 DexOpt: locking cache file %s (fd=%d, boot=%d)\n",
			  cacheFileName, fd, isBootstrap);
    ALOGV("DexOpt: locking cache file %s (fd=%d, boot=%d)",
        cacheFileName, fd, isBootstrap);
	printf("in Dexprepare.cpp:183\n");
    //ThreadStatus oldStatus = dvmChangeStatus(NULL, THREAD_VMWAIT);
	printf("in Dexprepare.cpp:185\n");
    //ZHKTODO cc = flock(fd, LOCK_EX | LOCK_NB);
	printf("in Dexprepare.cpp:187\n");
    /*
	if (cc != 0) {
		ALOGD("in Dexprepare.cpp:186 DexOpt: sleeping on flock(%s)\n", cacheFileName);
        ALOGD("DexOpt: sleeping on flock(%s)", cacheFileName);
        //TODOZHK cc = flock(fd, LOCK_EX);
    }*/
    //dvmChangeStatus(NULL, oldStatus);
    /**ZHKTODO
	if (cc != 0) {
		printf("ZHK in DexPrepare.cpp:195 Can't lock dex cache '%s': %d\n", cacheFileName, cc);
        ALOGE("Can't lock dex cache '%s': %d", cacheFileName, cc);
        close(fd);
        return -1;
    }
	*/
	printf("ZHK in Dexprepare.cpp:200 DexOpt:  locked cache file\n");
    ...
    if (fdStat.st_size == 0) {
        if (readOnly) {
			printf("ZHK in DexPrepare.cpp:220 DexOpt: file has zero length and isn't writable\n");
...
{
	printf("ZHK in DexPrepare.cpp:346 DexOpt: unlocking cache file fd=%d\n", fd);
    ...
    const char* fileName, u4 modWhen, u4 crc, bool isBootstrap)
{
     ...
     */
    if (gDvm.optimizing) {
		printf("ZHK in DexPrepare.cpp:393 Rejecting recursive optimization attempt on '%s'\n", fileName);
    ...
    u4 crc, bool expectVerify, bool expectOpt)
{
    ...
    int result = false;
    ssize_t actual;
	printf("ZHK dvmCheckOptHeaderAndDependencies:1242\n");
    /*
     * Start at the start.  The "opt" header, when present, will always be
     * the first thing in the file.
     */
    if (lseek(fd, 0, SEEK_SET) != 0) {
		printf("DexOpt: failed to seek to start of file: %s", strerror(errno));
        ALOGE("DexOpt: failed to seek to start of file: %s", strerror(errno));
        goto bail;
    }
	printf("ZHK dvmCheckOptHeaderAndDependencies:1252\n");
    /*
    ...
    if (actual < 0) {
		printf("DexOpt: failed reading opt header: %s", strerror(errno));
        ALOGE("DexOpt: failed reading opt header: %s", strerror(errno));
        goto bail;
    } else if (actual != sizeof(optHdr)) {
		printf("DexOpt: failed reading opt header (got %d of %zd)",
			  (int) actual, sizeof(optHdr));
        ALOGE("DexOpt: failed reading opt header (got %d of %zd)",
            (int) actual, sizeof(optHdr));
        goto bail;
    }
	printf("ZHK dvmCheckOptHeaderAndDependencies:1269\n");
    magic = optHdr.magic;
    if (memcmp(magic, DEX_MAGIC, 4) == 0) {
        /* somebody probably pointed us at the wrong file */
		printf("DexOpt: expected optimized DEX, found unoptimized");
		...
		printf("DexOpt: incorrect opt magic number (0x%02x %02x %02x %02x)",
			  magic[0], magic[1], magic[2], magic[3]);
        ALOGD("DexOpt: incorrect opt magic number (0x%02x %02x %02x %02x)",
            magic[0], magic[1], magic[2], magic[3]);
        goto bail;
    }
	printf("ZHK dvmCheckOptHeaderAndDependencies:1284\n");
    if (memcmp(magic+4, DEX_OPT_MAGIC_VERS, 4) != 0) {
		printf("DexOpt: stale opt version (0x%02x %02x %02x %02x)",
    ...
    if (optHdr.depsLength < kMinDepSize || optHdr.depsLength > kMaxDepSize) {
		printf("DexOpt: weird deps length %d, bailing", optHdr.depsLength);
	...
	printf("ZHK dvmCheckOptHeaderAndDependencies:1331\n");
    ...
    const u1* ptr;
    u4 val;
	printf("ZHK dvmCheckOptHeaderAndDependencies:1347\n");
    ptr = depData;
    val = read4LE(&ptr);
