...
jint JNI_CreateJavaVM(JavaVM** p_vm, JNIEnv** p_env, void* vm_args) {
	//return JNI_OK;
	printf("\tZHK JNI_CreateJavaVM:3412 begin...\n");
    const JavaVMInitArgs* args = (JavaVMInitArgs*) vm_args;
	if (args->version < JNI_VERSION_1_2) {
		// JNI_EVERSION -3
		printf("\tZHK in Jni.cpp:3416 return JNI_EVERSION(-3)\n");
        return JNI_EVERSION;
    }
	...
	printf("\tZHK JNI_CreateJavaVM:3432\n");
    ...
    for (int i = 0; i < args->nOptions; i++) {
        ...
        if (optStr == NULL) {
			...
			printf("\tZHK JNI_CreateJavaVM:3448\n");
            return JNI_ERR;
            ...
            for (size_t i = 0; i < jniOptCount; ++i) {
                if (strcmp(jniOpt, "warnonly") == 0) {
                    ...
                    dvmFprintf(stderr, "ERROR: CreateJavaVM failed: unknown -Xjniopts option '%s'\n",
                            jniOpt);
					printf("ZHK in Jni.cpp return JNI_ERR_#1(%d)\n", JNI_ERR);
                    return JNI_ERR;
                }
	...
	printf("\tZHK JNI_CreateJavaVM:3491\n");
    if (gDvmJni.useCheckJni) {
    ...
    if (gDvmJni.jniVm != NULL) {
		...
		printf("\tZHK in Jni.cpp return JNI_ERR_#2(%d)\n", JNI_ERR);
        return JNI_ERR;
    }
	...
	printf("\tZHK JNI_CreateJavaVM:3511\n");
    /* Initialize VM. */
    gDvm.initializing = true;
    std::string status =
            dvmStartup(argc, argv.get(), args->ignoreUnrecognized, (JNIEnv*)pEnv);
	printf("\tZHK JNI_CreateJavaVM:3516\n");
    ...
    if (!status.empty()) {
		...
		printf("\tZHK in Jni.cpp:3524 return JNI_ERR_#3(%d)\n", JNI_ERR);
        return JNI_ERR;
    }
	printf("\tZHK JNI_CreateJavaVM 3526\n");
    /*
    ...
    *p_vm = (JavaVM*) pVM;
	...
	printf("\tZHK in Jni.cpp return JNI_OK(%d)\n", JNI_OK);
    return JNI_OK;
}
