...
int dvmDexFileOpenFromFd(int fd, DvmDex** ppDvmDex)
{
    ...
    if (lseek(fd, 0, SEEK_SET) < 0) {
		printf("ZHK in DvmDex.cpp:103 lseek rewind failed\n");
        ALOGE("lseek rewind failed");
        goto bail;
    }

    if (sysMapFileInShmemWritableReadOnly(fd, &memMap) != 0) {
        printf("ZHK in DvmDex.cpp:109 Unable to map file\n");
		ALOGE("Unable to map file");
        goto bail;
    }
	printf("ZHK in DvmDex.cpp:113\n");
    pDexFile = dexFileParse((u1*)memMap.addr, memMap.length, parseFlags);
    printf("ZHK in DvmDex.cpp:115\n");
	if (pDexFile == NULL) {
		printf("ZHK in DvmDex.cpp:116 DEX parse failed\n");
