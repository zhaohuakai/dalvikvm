...
void *dvmAllocRegion(size_t byteCount, int prot, const char *name) {
	...
	//fd = open(name, O_RDWR);
	//ioctl(fd, ASHMEM_SET_SIZE, byteCount);
	
    //printf("ZHK in Misc.cpp fd=%d\n", fd);
	if (fd == -1) {
		printf("ZHK in Misc.cpp:614\n");
        return NULL;
    }
	//printf("in Misc.cpp before base\n");
    base = mmap(NULL, byteCount, prot, MAP_PRIVATE, fd, 0);
    //printf("in Misc.cpp after base\n");
	ret = close(fd);
	//printf("in Misc.cpp after close\n");
    if (base == MAP_FAILED) {
		printf("ZHK in Misc.cpp:620\n");
        return NULL;
    }
	//printf("in Misc.cpp ##1\n");
    if (ret == -1) {
		printf("ZHK in Misc.cpp:624\n");
        return NULL;
    }
	//printf("in Misc.cpp before return\n");
    return base;
}
