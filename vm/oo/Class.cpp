...
bool dvmClassStartup()
{
	...
	printf("\t\t\tZHK dvmClassStartup:436\n");
    /*
	...
	printf("\t\t\tZHK dvmClassStartup:445\n");
    /*
	...
	printf("\t\t\tZHK dvmClassStartup:453\n");
    /*
     * Process the bootstrap class path.  This means opening the specified
     * DEX or Jar files and possibly running them through the optimizer.
     */
    assert(gDvm.bootClassPath == NULL);
	printf("\t\t\tZHK dvmClassStartup:460 bootClassPathStr=%s\n", gDvm.bootClassPathStr);
    processClassPath(gDvm.bootClassPathStr, true);
	printf("\t\t\tZHK dvmClassStartup:462\n");
    if (gDvm.bootClassPath == NULL)
        return false;
	printf("\t\t\tZHK dvmClassStartup:465 end\n");
    return true;
}
...
static bool prepareCpe(ClassPathEntry* cpe, bool isBootstrap)
{
    ...
    if (stat(cpe->fileName, &sb) < 0) {
		printf("ZHK in Class.cpp:612 Unable to stat classpath element '%s'\n", cpe->fileName);
        ALOGD("Unable to stat classpath element '%s'", cpe->fileName);
        return false;
    }
    if (S_ISDIR(sb.st_mode)) {
		printf("ZHK in Class.cpp:617 Directory classpath elements are not supported: %s\n", cpe->fileName);
        ALOGE("Directory classpath elements are not supported: %s\n", cpe->fileName);
        return false;
    }

    char suffix[10];
    getFileNameSuffix(cpe->fileName, suffix, sizeof(suffix));
	printf("\t\t\t\t\tZHK in Class.cpp:624 suffix=%s\n", suffix);
    if ((strcmp(suffix, "jar") == 0) || (strcmp(suffix, "zip") == 0) ||
		...
		printf("ZHK in Class.cpp:641 Unknown type suffix '%s'", suffix);
        ALOGE("Unknown type suffix '%s'", suffix);
    }
	printf("ZHK in Class.cpp:644 Unable to process classpath element '%s'\n", cpe->fileName);
...
static ClassPathEntry* processClassPath(const char* pathStr, bool isBootstrap)
{
	...
	printf("\t\t\t\tZHK processClassPath:669 pathStr=%s\n", pathStr);
    /*
    ...
    cp = mangle;
    idx = 0;
	printf("\t\t\t\tZHK processClassPath:701\n");
    while (cp < end) {
        if (*cp == '\0') {
            ...
            if (isBootstrap &&
				...
				printf("\t\t\t\tZHK processClassPath:708 Non-absolute bootclasspath entry '%s'", cp);
			...
			printf("\t\t\t\tZHK processClassPath:717 filename=%s\n", cp);
            ...
            if (!prepareCpe(&tmp, isBootstrap)) {
				...
				printf("\t\t\t\tZHK processClassPath:736\n");
	...
	printf("\t\t\t\tZHK processClassPath:739\n");
    assert(idx <= count);
	printf("\t\t\t\tZHK processClassPath:741\n");
    if (idx == 0 && !gDvm.optimizing) {
         ...
         */
		printf("\t\t\t\tZHK processClassPath:747 No valid entries found in bootclasspath '%s'\n", pathStr);
        ALOGE("No valid entries found in bootclasspath '%s'", pathStr);
        gDvm.lastMessage = pathStr;
        dvmAbort();
    }
	printf("\t\t\t\tZHK processClassPath:750\n");
    ...
    cpe[idx].fileName = NULL;
    cpe[idx].ptr = NULL;
	printf("\t\t\t\tZHK processClassPath:755\n");
    //dumpClassPath(cpe);
...
{
    int result;
	printf("ZHK dvmFindLoadedClass:4638=%s\n", descriptor);
    dvmHashTableLock(gDvm.loadedClasses);
	printf("ZHK dvmFindLoadedClass:4640\n");
    result = dvmHashForeach(gDvm.loadedClasses, findClassCallback,
            (void*) descriptor);
	printf("ZHK dvmFindLoadedClass:4643\n");
    dvmHashTableUnlock(gDvm.loadedClasses);
	printf("ZHK dvmFindLoadedClass:4645\n");
...
{
	printf("ZHK dvmGetSystemClassLoader:4656\n");
    Thread* self = dvmThreadSelf();
	printf("ZHK dvmGetSystemClassLoader:4658\n");
    ClassObject* clClass = gDvm.classJavaLangClassLoader;
	//ZHKTODO
	printf("ZHK dvmGetSystemClassLoader:4659\n");
    if (!dvmIsClassInitialized(clClass) && !dvmInitClass(clClass))
        return NULL;
	printf("ZHK dvmGetSystemClassLoader:4662\n");
    JValue result;
	// ERRORTODO
    dvmCallMethod(self, gDvm.methJavaLangClassLoader_getSystemClassLoader,
        NULL, &result);
	printf("ZHK dvmGetSystemClassLoader:4668\n");
    Object* loader = (Object*)result.l;
    dvmAddTrackedAlloc(loader, self);
	printf("ZHK dvmGetSystemClassLoader:4670\n");
    return loader;
}
