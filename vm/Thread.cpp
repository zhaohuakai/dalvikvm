...
bool dvmPrepMainThread()
{
    ...
    Method* init;
    JValue unused;
	printf("+++ finishing prep on main VM thread\n");
    ALOGV("+++ finishing prep on main VM thread");
	printf("ZHK dvmPrepMainThread:692\n");
    /* main thread is always first in list at this point */
    ...
    if (!dvmInitClass(gDvm.classJavaLangClass)) {
		printf("'Class' class failed to initialize\n");
        ALOGE("'Class' class failed to initialize");
        return false;
    }
	printf("ZHK dvmPrepMainThread:706\n");
    ...
    {
		printf("thread classes failed to initialize\n");
        ALOGE("thread classes failed to initialize");
        return false;
    }
	printf("ZHK dvmPrepMainThread:715\n");
	...
	printf("ZHK dvmPrepMainThread:746\n");
    /*
	...
	printf("ZHK dvmPrepMainThread:757\n");
	...
	printf("ZHK dvmPrepMainThread:765\n");
	...
	printf("ZHK dvmPrepMainThread:779\n");
    /*
	...
	printf("ZHK dvmPrepMainThread:789\n");
    if (systemLoader == NULL) {
		printf("WARNING: system class loader is NULL (setting main ctxt)\n");
        ALOGW("WARNING: system class loader is NULL (setting main ctxt)");
        /* keep going? */
    } else {
		printf("ZHK dvmPrepMainThread:795\n");
        dvmSetFieldObject(threadObj, gDvm.offJavaLangThread_contextClassLoader,
            systemLoader);
		printf("ZHK dvmPrepMainThread:796\n");
        dvmReleaseTrackedAlloc(systemLoader, NULL);
    }
	printf("ZHK dvmPrepMainThread:797\n");
...
{
	//printf("ZHK dvmChangeStatus:2897\n");
    ThreadStatus oldStatus;

    if (self == NULL)
        self = dvmThreadSelf();
	//printf("ZHK dvmChangeStatus:2911\n");
	//printf("threadid=%d: (status %d -> %d)\n", self->threadId, self->status, newStatus);
    //LOGVV("threadid=%d: (status %d -> %d)",
    //    self->threadId, self->status, newStatus);

    oldStatus = self->status;
    if (oldStatus == newStatus){
		printf("ZHK dvmChangeStatus:2918 return oldStatus\n");
		return oldStatus;
	}
	//printf("ZHK dvmChangeStatus:2921\n");
    ...
    if (newStatus == THREAD_RUNNING) {
         ...
         */
		//printf("ZHK dvmChangeStatus:2964\n");
        volatile void* raw = reinterpret_cast<volatile void*>(&self->status);
        volatile int32_t* addr = reinterpret_cast<volatile int32_t*>(raw);
        android_atomic_acquire_store(newStatus, addr);
		//printf("ZHK dvmChangeStatus:2967\n");
        if (self->suspendCount != 0) {
