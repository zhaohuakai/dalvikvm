...
bool dvmLoadNativeCode(const char* pathName, Object* classLoader,
        char** detail)
{
	printf("ZHK dvmLoadNativeCode:321\n");
    SharedLib* pEntry;
    void* handle;
    bool verbose;
	...
	printf("ZHK dvmLoadNativeCode:329\n");
    if (verbose)
    ...
    *detail = NULL;
	...
	printf("ZHK dvmLoadNativeCode:354\n");
    /*
	...
	printf("ZHK dvmLoadNativeCode:401\n");
    ...
    if (pNewEntry != pActualEntry) {
        ...
        void* vonLoad;
        int version;
		printf("ZHK dvmLoadNativeCode:417\n");
        vonLoad = dlsym(handle, "JNI_OnLoad");
		printf("ZHK dvmLoadNativeCode:419\n");
        if (vonLoad == NULL) {
			printf("No JNI_OnLoad found in %s %p, skipping init\n",
				  pathName, classLoader);
            ALOGD("No JNI_OnLoad found in %s %p, skipping init",
                pathName, classLoader);
        } else {
			printf("ZHK dvmLoadNativeCode:424\n");
            /*
			...
			printf("ZHK dvmLoadNativeCode:435\n");
            self->classLoaderOverride = classLoader;
            oldStatus = dvmChangeStatus(self, THREAD_NATIVE);
            if (gDvm.verboseJni) {
				printf("[Calling JNI_OnLoad for \"%s\"]", pathName);
                ALOGI("[Calling JNI_OnLoad for \"%s\"]", pathName);
            }
			printf("ZHK dvmLoadNativeCode:442\n");
            version = (*func)(gDvmJni.jniVm, NULL);
            dvmChangeStatus(self, oldStatus);
            self->classLoaderOverride = prevOverride;
			printf("ZHK dvmLoadNativeCode:446\n");
            if (version != JNI_VERSION_1_2 && version != JNI_VERSION_1_4 &&
                version != JNI_VERSION_1_6)
            {
				printf("JNI_OnLoad returned bad version (%d) in %s %p",
