...
{
	printf("\t\t\tZHK processOptions:655 begin...\n");
    int i;
    ...
    for (i = 0; i < argc; i++) {
                   ...
                   strcmp(argv[i], "-cp") == 0)
        {
			...
			//printf("ZHK ------------- in Init.cpp:655 calsspath=%s\n", gDvm.classPathStr);
                ...
                sizeof("-Xbootclasspath:")-1) == 0)
        {
			...
			//printf("ZHK ----------- in Init.cpp:711 gDvm.bootClassPathStr=%s\n", gDvm.bootClassPathStr);
        ...
        bool ignoreUnrecognized, JNIEnv* pEnv)
{
	...
	printf("\t\tZHK dvmStartup:1206 begin...\n");
    ALOGV("VM init args (%d):", argc);
    for (int i = 0; i < argc; i++) {
        ALOGV("  %d: '%s'", i, argv[i]);
    }
    setCommandLineDefaults();
	printf("\t\tZHK dvmStartup:1212\n");
    /*
    ...
    if (!dvmGcStartup()) {
		// ZHK first error
		printf("\t\tZHK dvmStartup:1260  dvmGcStartup failed\n");
        return "dvmGcStartup failed";
    }
	printf("\t\tZHK dvmStartup:1263\n");
    if (!dvmThreadStartup()) {
        return "dvmThreadStartup failed";
    }
	printf("\t\tZHK dvmStartup:1267\n");
    if (!dvmInlineNativeStartup()) {
        return "dvmInlineNativeStartup";
    }
	printf("\t\tZHK dvmStartup:1271\n");
    if (!dvmRegisterMapStartup()) {
        return "dvmRegisterMapStartup failed";
    }
	printf("\t\tZHK dvmStartup:1275\n");
    if (!dvmInstanceofStartup()) {
        return "dvmInstanceofStartup failed";
    }
	printf("\t\tZHK dvmStartup:1279\n");
    if (!dvmClassStartup()) {
		printf("ZHK in Init.cpp:1281 dvmClassStartup failed\n");
        return "dvmClassStartup failed";
    }
	printf("\t\tZHK dvmStartup:1283\n");
    /*
	...
	printf("\t\tZHK dvmStartup:1382\n");
    /* general debugging setup */
    if (!dvmDebuggerStartup()) {
        return "dvmDebuggerStartup failed";
    }
	printf("\t\tZHK dvmStartup:1387\n");
    if (!dvmGcStartupClasses()) {
        return "dvmGcStartupClasses failed";
    }
	printf("\t\tZHK dvmStartup:1391\n");
    /*
...
static bool registerSystemNatives(JNIEnv* pEnv)
{
    ...
    printf("ZHK in Init.cpp:1459\n");
	loadJniLibrary("javacore");
	printf("ZHK in Init.cpp:1461\n");
    loadJniLibrary("nativehelper");
	printf("ZHK in Init.cpp:1463\n");
    // Back to run mode.
