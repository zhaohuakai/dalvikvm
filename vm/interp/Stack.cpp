...
static bool dvmPopFrame(Thread* self)
{
	...
	ALOGV("POP frame: cur=%p new=%p",
		  self->interpSave.curFrame, saveBlock->prevFrame);
    LOGVV("POP frame: cur=%p new=%p",
        self->interpSave.curFrame, saveBlock->prevFrame);

    self->interpSave.curFrame = saveBlock->prevFrame;
	//printf("\t\t\tdvmPopFrame:347\n");
    return true;
}
...
{
	printf("~~~~~~~~~ZHK in dvmCallMethodV:448\n");
    const char* desc = &(method->shorty[1]); // [0] is the return type.
    int verifyCount = 0;
    ClassObject* clazz;
    u4* ins;
	printf("~~~~~~~ZHK in dvmCallMethodV:456\n");
    clazz = callPrep(self, method, obj, false);
    if (clazz == NULL)
        return;
	printf("~~~~~~~ZHK in dvmCallMethodV:460\n");
    /* "ins" for new frame start at frame pointer plus locals */
    ins = ((u4*)self->interpSave.curFrame) +
           (method->registersSize - method->insSize);

	 printf("  FP is %p, INs live at >= %p\n", self->interpSave.curFrame, ins);
    //ALOGV("  FP is %p, INs live at >= %p", self->interpSave.curFrame, ins);
	...
	printf("~~~~~~~ZHK in dvmCallMethodV:475\n");
    while (*desc != '\0') {
        switch (*(desc++)) {
            case 'D': case 'J': {
	...
	printf("~~~~~~~ZHK in dvmCallMethodV:504\n");
#ifndef NDEBUG
    if (verifyCount != method->insSize) {
		ALOGV("Got vfycount=%d insSize=%d for %s.%s", verifyCount,
    ...
    if (dvmIsNativeMethod(method)) {
		printf("~~~~~~~ZHK in dvmCallMethodV:526\n");
		...
		printf("~~~~~~~ZHK in dvmCallMethodV:527\n");
		//printf("ZHK in dvmCallMethodV:529\n");
        dvmInterpret(self, method, pResult);
    }
	printf("~~~~~~~ZHK in dvmCallMethodV:528\n");
#ifndef NDEBUG
bail:
#endif
    dvmPopFrame(self);
	printf("~~~~~~ZHK in dvmCallMethodV:533\n");
}
